...
DEMO_DATA_CONFIG = {
    ...
    'partners': [
        ...
        {'name': 'Vendor A', 'email': 'vendor.a@example.com',
            'is_company': True, 'supplier_rank': 1},
        {'name': 'Vendor B', 'email': 'vendor.b@example.com',
            'is_company': True, 'supplier_rank': 1}
    ],
    'products': [
...
class DatabaseSetup:
    ...
    def _create_postgresql_database(self, database_name: str) -> None:
        ...
        try:            # Kết nối đến PostgreSQL
            conn = psycopg2.connect(
                host='localhost',  # Use localhost instead of container name
                port=self.config.postgresql.port,
                user=self.config.postgresql.user,  # Use regular user instead of admin_user
    ...
    def _install_odoo_modules(self, web_url: str, database_name: str, version: str) -> List[str]:
        ...
        # Chỉ cài modules có sẵn        filtered_modules = [m for m in modules_to_install if m in available_modules]
        ...
        for i in range(0, len(filtered_modules), batch_size):
            ...
            try:
                result = self.module_installer.install_modules(
    ...
    def _check_single_database(self, version: str) -> Dict[str, Any]:
        ...
        try:            # Kiểm tra database tồn tại trong PostgreSQL
            conn = psycopg2.connect(
                host='localhost',
                port=self.config.postgresql.port,
    ...
    def _count_installed_modules(self, database_name: str, version: str) -> int:
        ...
        try:            # Kết nối trực tiếp đến database để đếm modules
            conn = psycopg2.connect(
                host='localhost',
                port=self.config.postgresql.port,
                user=self.config.postgresql.user,
    ...
    def _cleanup_single_database(self, version: str, force: bool = False) -> Dict[str, Any]:
        ...
        try:            # Kiểm tra database có tồn tại không
            conn = psycopg2.connect(
                host='localhost',
                port=self.config.postgresql.port,
                user=self.config.postgresql.user,
